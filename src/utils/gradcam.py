...
def build_guided_model(build_model_fn):
    ...
    """
    if "GuidedBackProp" not in tf_ops._gradient_registry._registry:
        @tf_ops.RegisterGradient("GuidedBackProp")
...
def grad_cam_batch(input_model, images, classes, layer_name):
    ...
    loss = tf.gather_nd(input_model.output, np.dstack(
        [range(images.shape[0]), classes])[0])
    layer_output = input_model.get_layer(layer_name).output
...
def compute_saliency(model, guided_model, img_path, layer_name='block5_conv3', visualize=True, save=True, classes=None):
    ...
    top_n = 5
    top = decode_predictions(classes, predictions[0], len(classes))
    ...
    gradcam = grad_cam(model, preprocessed_input, -1, layer_name)
