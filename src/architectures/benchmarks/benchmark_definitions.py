...
class Chestxray14_Benchmark(Benchmark):
    def __init__(self, name, train_labels="meta/data/labels.csv", split_group='Patient ID',
        ...
        super(Chestxray14_Benchmark, self).__init__(Path(os.environ.get("CHESTXRAY14_DATASET_DIRECTORY")),
                                                  CHESTXRAY14_COLUMNS,
                                                  name,
                                                  train_labels=train_labels,
                                                  split_group = split_group,
...
for batch_name, batch_size in BATCH_SIZES.items():                             
    for epoch_name, epoch_size in EPOCH_SIZES.items():
        key_suffix = "_E{epoch_size}_B{batch_size}".format(epoch_size=epoch_size, batch_size=batch_size)  
        name_suffix = "{epoch_name} Epochs and {batch_name} Batches".format(epoch_name=epoch_name, batch_name=batch_name)        
        
        CHEXPERT_BENCHMARKS["BCE" + key_suffix] = Chexpert_Benchmark("Chexpert BCE "+name_suffix,
                                                                      epochs= epoch_size,
