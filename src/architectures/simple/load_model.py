...
def reevaluate_on_datagen(exp, datagen, new_metrics=False):
    ...
    for metric in metrics:
        metric.reset_states()
        metric.update_state(tensorflow.constant(exp.groundtruth_label), tensorflow.constant(exp.predictions))
...
def get_experiment_from_logs(name=None, experiment_id=None, version="1", basepath=Path("/srv/idp-radio-1")):
    ...
    if name is not None:
        experiments_with_name = [exp for exp in experiments if exp["name"] == name and exp["version"] == version]
        if len(experiments_with_name) != 0:
            return experiments_with_name[0]
    
    if experiment_id is not None:
        experiments_with_id = [exp for exp in experiments if exp["id"] == experiment_id and exp["version"] == version]
...
def benchmark_from_logs(experiment_dict):
    ...
    return Benchmark(Path(benchmark_dict["dataset_folder"]),
                          ...
                          preprocess_input_fn = get_preprocessing_for_architecture(experiment_dict["name"]),
...
def rebuild_experiment(experiment_dict, epoch = None, model_folder=Path("/srv/idp-radio-1/models")):
    ...
    experiment = simple_architecture_experiment(benchmark, architecture_fn, benchmark.label_columns)
